...
const TEXT = {
    ...
    skipped: 'No removable watermark detected — original preserved',
    unsupported: 'Your browser does not support clipboard image copy',
    copied: 'Copied!',
    copy: 'Copy Result',
    copyFailed: 'Copy failed — check browser permissions',
    unsupportedFile: 'Please select a JPG, PNG, or WebP image, or an MP4/WebM/MOV video.',
    fileTooLarge: 'Images over 20 MB are not processed here. Videos will enter the video flow.',
    skippedLargeImages: 'Some images over 20 MB were skipped.',
    handoffVideo: 'Opening video processing flow...',
    progress: 'Progress',
    pending: 'Waiting',
...
async function init() {
    try {
        ...
        if (badge) {
            ...
            if (workerClient) {
                badge.setAttribute('data-mode', 'worker');
                badge.textContent = '⚡ Worker Accelerated';
            } else {
                badge.setAttribute('data-mode', 'main');
                badge.textContent = '🐢 Main Thread';
            }
        }
...
async function routeVideoFile(file) {
    try {
        ...
        hideLoading();
        console.error(error);
        setStatusMessage(error.message || 'Failed to open video processing flow. Please open the video page and reselect.', 'warn');
    }
}
...
async function consumePendingImageHandoff() {
    ...
    try {
        ...
        setStatusMessage(error.message || 'Failed to load image handoff. Please reselect the file.', 'warn');
    }
}
...
function renderSingleImageMeta(item) {
    ...
    // Update alt text now that we know the image
    const origEl = document.getElementById('originalImage');
    if (origEl) origEl.alt = `Original image: ${item.name}`;
...
async function processSingle(item) {
    try {
        ...
        processedImage.alt = `Processed image after watermark removal: ${item.name}`;
...
function setupSlider() {
    ...
    // Keyboard support (arrow keys move slider by 5%)
    container.addEventListener('keydown', (e) => {
        const current = parseFloat(processedOverlay.style.width) || 50;
        if (e.key === 'ArrowLeft') {
            e.preventDefault();
