...
function createRuntimeCanvas(width, height) {
    ...
    throw new Error('No Canvas available in this environment.');
}
...
/**
 * Compute scaled output dimensions that fit within maxWidth x maxHeight,
 ...
 */
export function resolveScaledDimensions(srcW, srcH, maxW, maxH) {
    if (srcW <= maxW && srcH <= maxH) {
        // Source already fits — no downscale needed
        return { width: srcW, height: srcH };
    }
    const scaleW = maxW / srcW;
    const scaleH = maxH / srcH;
    const scale = Math.min(scaleW, scaleH);
    // Round to even numbers (required by most H.264 encoders)
    return {
...
async function getVideoContext(file) {
    ...
    if (!videoTrack) {
        input.dispose();
        throw new Error('No processable video track found in this file.');
    }
...
async function copyAudioPackets(audioCopy, options = {}) {
    ...
    try {
        while (currentTotalTime < targetDuration) {
            ...
            for await (const packet of sink.packets()) {
                ...
                const finalTs = currentTotalTime + normalized.timestamp;
                if (finalTs >= targetDuration) break;

                const finalDur = Math.min(normalized.duration, targetDuration - finalTs);
                const pkt = normalized.clone({
                    timestamp: finalTs,
                    duration: finalDur
                });
                await audioCopy.source.add(pkt, audioCopy.meta);
                packetCount++;

                cycleDuration = Math.max(cycleDuration, normalized.timestamp + normalized.duration);
            }

            if (cycleDuration <= 0 || !options?.loopEnabled) break;
            currentTotalTime += cycleDuration;
            cycleCount++;
            if (cycleCount >= 10000) break;
        }
...
export async function detectGeminiVideoWatermark(file, options = {}) {
    ...
    try {
        ...
        if (!frames.length) {
            input.dispose();
            throw new Error('Failed to extract detection frames from video.');
        }
...
export async function removeGeminiVideoWatermark(file, options = {}) {
    ...
    if (!detection.isConfident && options.allowLowConfidence !== true) {
        throw new Error('Watermark detection confidence is too low. Enable low-confidence export to proceed.');
    }

    // Resolve output dimensions — never upscale, always maintain aspect ratio
    const outputDims = (() => {
        const maxW = Number(options.outputWidth);
        const maxH = Number(options.outputHeight);
        if (Number.isFinite(maxW) && maxW > 0 && Number.isFinite(maxH) && maxH > 0) {
            return resolveScaledDimensions(metadata.width, metadata.height, maxW, maxH);
        }
        return { width: metadata.width, height: metadata.height };
    })();
    const needsScale = outputDims.width !== metadata.width || outputDims.height !== metadata.height;
    ...
    if (!canEncodeAvc) {
        throw new Error('H.264/AVC encoding is not supported in this browser. Please use a recent version of Chrome or Edge.');
    }

    const { input, videoTrack } = await getVideoContext(file);
    // Source canvas: always full resolution (watermark removal accuracy)
    const canvas = createRuntimeCanvas(metadata.width, metadata.height);
    const ctx = get2dContext(canvas);
    // Output canvas: scaled to target resolution (may equal source)
    const outputCanvas = needsScale
        ? createRuntimeCanvas(outputDims.width, outputDims.height)
        : canvas;
    ...
    try {
        await output.start();
        const audioCopyPromise = copyAudioPackets(audioCopy, options);
        ...
        let currentTotalTime = 0;
        let cycleCount = 0;
        let lastTimestampInCycle = 0;

        while (currentTotalTime < targetDuration) {
            const sink = new VideoSampleSink(videoTrack);
            let cycleDuration = 0;
            lastTimestampInCycle = 0;

            for await (const sample of sink.samples()) {
                let timestamp = Math.max(0, sample.timestamp - metadata.firstTimestamp);
                if (timestamp < lastTimestampInCycle) {
                    timestamp = lastTimestampInCycle + fallbackDuration;
                }
                const duration = Number.isFinite(sample.duration) && sample.duration > 0
                    ? sample.duration
                    : fallbackDuration;

                const finalTs = currentTotalTime + timestamp;
                if (finalTs >= targetDuration) {
                    sample.close();
                    break;
                }
                ...
                // Downscale to output canvas if target resolution differs from source
                if (needsScale) {
                    const outputCtx = outputCanvas.getContext('2d', { willReadFrequently: false });
                    if (outputCtx) {
                        outputCtx.drawImage(canvas, 0, 0, outputDims.width, outputDims.height);
                    }
                }

                await source.add(finalTs, duration);
                lastTimestampInCycle = timestamp;
                lastTimestamp = finalTs;
                processedFrames++;
                cycleDuration = Math.max(cycleDuration, timestamp + duration);

                const baseFrameEstimate = metadata.frameCountEstimate || Math.max(1, Math.round((metadata.duration || 0) * metadata.frameRate));
                const frameEstimate = (options?.loopEnabled && options?.loopDurationSeconds > 0 && metadata.duration > 0)
                    ? Math.round(baseFrameEstimate * (options.loopDurationSeconds / metadata.duration))
                    : baseFrameEstimate;
                const totalExpectedDuration = (options?.loopEnabled && options?.loopDurationSeconds > 0)
                    ? options.loopDurationSeconds
                    : metadata.duration;
                const elapsedSeconds = finalTs + duration;
                const timeProgress = Number.isFinite(totalExpectedDuration) && totalExpectedDuration > 0
                    ? Math.max(0, Math.min(1, elapsedSeconds / totalExpectedDuration))
                    : null;
                onProgress({
                    phase: 'export',
        ...
        if (!target.buffer) {
            throw new Error('Video export failed: output is empty.');
        }
